...
const spys = {
    ...
    getProxies: async urlsAndCountries => {
        for (const urlAndCountry of urlsAndCountries) {
            ...
            let jsonData = []
            try { jsonData = JSON.parse(fs.readFileSync('./media/proxies_spys.json', 'utf-8')) } // read file with saved proxies
            catch (err) { fs.writeFileSync('./media/proxies_spys.json', JSON.stringify(jsonData)) } // create new file if there the one doesn't exist
